...
const AgentProfile = () => {
  ...
  useEffect(() => {
    ...
    console.log("Raw stored data:", storedAgentData);
    console.log("Type of stored data:", typeof storedAgentData);

    if (storedAgentData) {
      try {
        // First, try to parse the stored data
        const parsed = JSON.parse(storedAgentData);
        console.log("Successfully parsed data:", parsed);

        // Then access the payload
        const agentPayload = parsed?.payload;

        if (agentPayload) {
          setAgentData(agentPayload);
          setPhoneNumber(agentPayload.phoneNumber || "");
          setFirstName(agentPayload.firstname || "");
          setLastName(agentPayload.lastname || "");
        } else {
          console.error("Payload not found in agentData");
        }
      } catch (error) {
        console.error("Error parsing agentData from localStorage:", error);
        // Also log the problematic string
        console.log("Problematic JSON string:", storedAgentData.slice(0, 100));
      }
    }
  }, []);
  ...
  return (
    <>
      <Sidebar
        ...
        loading={loading}
      />
      <div className="flex flex-col lg:flex-row mt-[5rem] max-w-[1100px] w-[95%] mx-auto md:mx-16 gap-10 min-h-screen p-4 lg:ml-64 xl:ml-64">
        {/* Sidebar */}
